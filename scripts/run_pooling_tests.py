...
def find_test_files() -> List[str]:
    ...
    all_tests = pooling_tests + [
        f for f in additional_tests if os.path.exists(f)
    ]
...
def discover_bfloat16_tests(test_files: List[str]) -> Set[str]:
    ...
    for test_file in test_files:
        ...
        try:
            ...
            print(
                f"⚠️  Warning: Could not collect tests from {test_file}: {e}")
            continue
...
def run_test_file(test_file: str,
                  bfloat16_tests: Set[str]) -> Tuple[str, List[str]]:
...
def write_summary_report(all_results: List[Tuple[str, str, List[str]]],
                         summary_file: str):
...
def main():
    ...
    for test_file, log_file, results in all_results:
        ...
        print(
            f"{status_icon} {Path(test_file).name}: {passed} passed, {failed} failed"
        )
    ...
    print("   python test_log_summarizer.py test_output_*.log")
