...
def extract_oom_details(error_msg):
    ...
    total_match = re.search(r'total capacity of ([\d.]+)\s*(GiB|MiB)',
                            error_msg)
    if total_match:
    ...
    used_match = re.search(
        r'this process has ([\d.]+)\s*(GiB|MiB) memory in use', error_msg)
    if used_match:
...
def detect_cuda_oom_in_logs(log_content: str, test_name: str) -> bool:
    ...
    oom_patterns = [
        r'CUDA out of memory', r'OutOfMemoryError',
        r'RuntimeError.*CUDA out of memory', r'torch\.cuda\.OutOfMemoryError',
        r'CUDA_ERROR_OUT_OF_MEMORY', r'out of memory.*cuda',
    ...
    while i < len(lines):
        ...
        if match:
            ...
            while j < min(len(lines), i + 200):
                ...
                if status_line in [
                        'PASSED', 'FAILED', 'SKIPPED', 'ERROR', 'XFAIL',
                        'XPASS'
                ]:
                    # Skip SKIPPED tests unless include_skipped is True
                    ...
                    if status in ['FAILED', 'ERROR']:
                        ...
                        if test_name in summary_info:
                            error_msg = summary_info[test_name]
                            if any(pattern in error_msg.lower()
                                   for pattern in [
                                       'cuda out of memory',
                                       'outofmemoryerror',
                                       'torch.outofmemoryerror'
                ...
                if re.match(test_with_logs_pattern,
                            status_line) and '::test_' in status_line:
                    break
...
def process_file(filepath, validate=False, include_skipped=False):
    ...
    try:
        ...
        if summary_text:
            validation = {}
            total_summary = summary_counts['passed'] + summary_counts[
                'failed'] + summary_counts['skipped'] + summary_counts['error']
            total_parsed = sum([
                v for k, v in parsed_counts.items() if k != 'oom'
            ])  # Don't double-count OOM
            ...
            if include_skipped:
                validation['counts_match'] = (
                    parsed_counts.get('passed', 0) == summary_counts['passed']
                    and parsed_counts.get('failed', 0)
                    == summary_counts['failed'] and parsed_counts.get(
                        'skipped', 0) == summary_counts['skipped'] and
        ...
        if validate:
            ...
            if test_path:
                ...
                if all_tests:
                    file_result['validation']['all_available_tests'] = len(
                        all_tests)
                    file_result['validation'][
                        'pytest_collection'] = all_tests[:
                                                         10]  # Show first 10 as sample
...
def main():
    parser = argparse.ArgumentParser(
        description=
        'Extract simple test results from pytest logs with validation and OOM detection'
    )
    ...
    parser.add_argument('--validate',
                        action='store_true',
                        help='Validate against pytest collection')
    parser.add_argument('--summary-only',
                        action='store_true',
                        help='Show only summary validation')
    parser.add_argument('--include-skipped',
                        action='store_true',
                        help='Include SKIPPED tests in output')
    parser.add_argument('--with-emojis',
                        action='store_true',
                        help='Add emoji indicators to results')
    parser.add_argument('--oom-details',
                        action='store_true',
                        help='Show detailed memory info for OOM failures')
    ...
    for filepath in all_files:
        file_result = process_file(filepath, args.validate,
        ...
        if args.summary_only:
            ...
            if summary_text:
                ...
                all_results.append(
                    f"Parsed: {parsed_display} (Total: {sum([v for k, v in parsed_counts.items() if k != 'oom'])})"
                )
                ...
                if validation.get('counts_match'):
                    ...
                    if args.include_skipped:
                        all_results.append(
                            f"  Expected: P={summary_counts['passed']} F={summary_counts['failed']} S={summary_counts['skipped']} E={summary_counts['error']}"
                        )
                        all_results.append(
                            f"  Found:    P={parsed_counts.get('passed',0)} F={parsed_counts.get('failed',0)} S={parsed_counts.get('skipped',0)} E={parsed_counts.get('error',0)}"
                        )
                    else:
                        all_results.append(
                            f"  Expected (excluding skipped): P={summary_counts['passed']} F={summary_counts['failed']} E={summary_counts['error']}"
                        )
                        all_results.append(
                            f"  Found:    P={parsed_counts.get('passed',0)} F={parsed_counts.get('failed',0)} E={parsed_counts.get('error',0)}"
                        )

                    if parsed_counts.get('oom', 0) > 0:
                        all_results.append(
                            f"  OOM failures: {parsed_counts['oom']} (subset of failed/error)"
                        )
            else:
            ...
            if results:
                if args.with_emojis:
                    formatted_results = [
                        format_result_with_emoji(result) for result in results
                    ]
                ...
                if oom_count > 0:
                    ...
                    if args.oom_details:
                        ...
                        for test in oom_tests:
                            ...
                            if test_name in summary_info:
                                oom_details = extract_oom_details(
                                    summary_info[test_name])
                                if oom_details:
                                    if 'attempted' in oom_details:
                                        all_results.append(
                                            f"#      Attempted: {oom_details['attempted']}"
                                        )
                                    if 'free' in oom_details:
                                        all_results.append(
                                            f"#      Available: {oom_details['free']}"
                                        )
                                    if 'total' in oom_details:
                                        all_results.append(
                                            f"#      Total GPU: {oom_details['total']}"
                                        )
                                    if 'used' in oom_details:
                                        all_results.append(
                                            f"#      In Use: {oom_details['used']}"
                                        )
                    else:
                        for test in oom_tests:
                            test_name = test.split(' - ')[0]
            ...
            if args.verbose and summary_text:
                ...
                if 'all_available_tests' in validation:
                    all_results.append(
                        f"# Available tests in suite: {validation['all_available_tests']}"
                    )
    ...
    if not all_results:
        if args.include_skipped:
            ...
            print(
                "No passed/failed test results found in any files (skipped tests excluded)."
            )
        return
